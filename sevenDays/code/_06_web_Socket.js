		...
		function(conn) {
			conn.on('data', function(data) {
				conn.write([
					...
					'Content-Length: 11', '',		//so NodeJS就不会自动添加Transfer-Encoding字段和使用chunked传输方式。
					'Hello World' ].join('\n'));
			});
		}).listen(8080);

// 接着我们来看一个使用Socket发起HTTP客户端请求的例子。这个例子中，Socket客户端在建立连接后发送了一个HTTP
// GET请求，并通过data事件监听函数来获取服务器响应。
 ...
 * 答：因为默认情况下，使用.writeHead方法写入响应头后，允许使用.write方法写入任意长度的响应体数据，并使用.end方法结束一个响应。由于响应体数据长度不确定，因此NodeJS自动在响应头里添加了Transfer-Encoding:
 * chunked字段，并采用chunked传输方式。但是当响应体数据长度确定时，可使用.writeHead方法在响应头里加上Content-Length字段，这样做之后NodeJS就不会自动添加Transfer-Encoding字段和使用chunked传输方式。
 * 
 * 问： 为什么使用http模块发起HTTP客户端请求时，有时候会发生socket hang up错误?
 * 答：发起客户端HTTP请求前需要先创建一个客户端。
 * http模块提供了一个全局客户端: 
 * 		http.globalAgent , 
 * 可以让我们使用.request或.get方法时不用手动创建客户端。
